...
In order to use an STM for read/write operations, one must first create the STM itself. Reference to the STM instance will be used by all other functions. STM itself is of a special [type](data-types.md) `stm`.
...
Note that passing a zero argument lambda [requires](syntax.md#anonymous-functions-aka-lambdas) it to be wrapped in another lambda, this is to prevent its eager evaluation.
